...
O problema apresentado neste exercíciio é mais simples por poder ser reduzido a um vetor tal que:
...
A amostra pode ser aproximada pela distribuição normal dado que
$ np = `r np` \gt 5 \land nq = `r nq` \gt 5 $.
...
$$ p \approx 0.044 < \alpha = 0.1 \Longrightarrow \text{ Rejeita } H_0$$
...
- $$ \text{pH: }&\alpha = 0.05,\quad df = 39 - 1 = 38 $$
- $$ \text{Cond: }&\alpha = 0.01,\quad df = 39 - 1 = 38 $$
...
pairs <- data.frame(x = seq(-4, 4, length.out = 400))
...
- pH ($\alpha = 0.05$)
  - $p_1 \approx 0.081 > 0.05 \Longrightarrow$ Não rejeita $H_0$
- Condutividade ($\alpha = 0.01$)
  - $p_2 \approx 0.0029 > 0.01 \Longrightarrow$ Rejeita $H_0$
